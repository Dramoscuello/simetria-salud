...
Namespace Models
    Public Class Validacion1479
        ...
        Sub new_()
            ...
            dt = New DataTable
        End Sub
        Public Function Total_Facturado(ByVal IdUsuariA As String) As DataTable
            new_()
            Try
                Using conn As New MySqlConnection(conexion)
                    ...
                    Using cmd As New MySqlCommand("1479_Procedure", conn)
                        ...
                        cmd.Parameters.Add("op", MySqlDbType.VarChar).Value = "3"
                        cmd.Parameters.Add("NIT", MySqlDbType.VarChar).Value = "3"
            ...
            Try
                Using conn As New MySqlConnection(conexion)
                    If conn.State = ConnectionState.Closed Then
                        conn.Open()
                    End If
                    Using cmd As New MySqlCommand("Listar_Error_Estructura_1479", conn)
                        cmd.CommandType = CommandType.StoredProcedure
                        cmd.CommandTimeout = 900000000
                        cmd.Parameters.Add("USession", MySqlDbType.VarChar).Value = IdUsuariA
                        da.SelectCommand = cmd
            ...
            Try
                Using conn As New MySqlConnection(conexion)
                    If conn.State = ConnectionState.Closed Then
                        conn.Open()
                    End If
                    Using cmd As New MySqlCommand("Listar_Error_Detalle_1479", conn)
                        cmd.CommandType = CommandType.StoredProcedure
                        cmd.CommandTimeout = 900000000
                        cmd.Parameters.Add("USession", MySqlDbType.VarChar).Value = IdUsuariA
                        da.SelectCommand = cmd
